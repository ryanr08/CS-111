...
int main(int argc, char** argv)
{
	...
	if (SSL_library_init() < 0){
		fprintf(stderr, "Error loading OpenSSL library\n");
		exit(2);
	}
	...
	{
		fprintf(stderr, "Error starting TLS\n");
		exit(2);
	}
	...
	while(1)
	{
		...
		if (running && clock.tv_sec >= next)
		{
			...
			sprintf(tempBuff, "%02d:%02d:%02d %0.1f\n", currentTime->tm_hour, currentTime->tm_min, currentTime->tm_sec, temperature);
			SSL_write(ssl, tempBuff, strlen(tempBuff));
		...
		if (fds[0].revents & POLLIN)
		{
			...
			else if (!strcmp(buff2, "OFF\n"))
			{
				...
				char shutdownBuff[100];

				sprintf(shutdownBuff, "%02d:%02d:%02d SHUTDOWN\n", currentTime->tm_hour, currentTime->tm_min, currentTime->tm_sec);
			
				SSL_write(ssl, shutdownBuff, strlen(shutdownBuff));
