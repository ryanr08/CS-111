...
void *listRoutine(void *arg)
{
    ...
    for (; j < (*threadNum + 1) * iterations; j++)
    {
        if (listOption[0] == 'N')
        {
            element = SortedList_lookup(list, elements[j].key);
            if (element == NULL)
            {
...
int main(int argc, char **argv)
{
    ...
    fprintf(stdout, "%s,%d,%d,%d,%ld,%ld,%ld\n", testname, threads, iterations, numLists, operations, runtime, time_per_operation);
